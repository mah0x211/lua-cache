...
function testcase.set()
    ...
    local err = assert.throws(c.set, c, 'foo bar')
...
function testcase.get()
    ...
    -- test that get a value associated with key and extends the lifetime
    assert.equal(c:get('foo', true), 'hello')
    ...
    assert.match(err, 'touch must be boolean')
end
